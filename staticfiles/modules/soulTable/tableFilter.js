...
layui.define(['table', 'form', 'laydate', 'util', 'excel', 'laytpl'], function (exports) {
  ...
  var mod = {
      ...
      if ($('#soul-filter-list' + tableId).length === 0) {
        ...
        $('#main-list' + tableId + ' .soul-condition').on('mouseover', function (e) {
          ...
          if (_this.startsWith(filterType, 'date')) {
              ...
              for (i = 0; i < filterSo.children.length; i++) {
                ...
                if (i === 0) {
                  ...
                  conditionHtml.push(
                    '<td>' +
                    '   <div>' +
                    '      <input type="checkbox" name="switch" lay-filter="soul-coondition-switch" lay-skin="switch" style="background-color: #2D93CA" lay-text="与|或" ' + (!prefix || prefix === 'and' ? 'checked' : '') + '>' +
                    '    </div>' +
                    '</td>')
                }
            ...
            conditionHtml.push('</tbody></table><div style="text-align: center; padding-top: 5px"><button class="layui-btn layui-btn-sm" data-type="add"><i class="layui-icon">&#xe654;</i>添加</button><span style="display: inline-block;width: 50px"></span><button class="layui-btn layui-btn-sm" data-type="search" style="background-color: #2D93CA"><i class="layui-icon">&#xe615;</i>查询</button></div>')
    ...
    showConditionBoard: function (myTable) {
      ...
      filterBoard.push('<div><a class="layui-btn layui-btn-sm" data-type="addOne" style="background-color: #2D93CA"><i class="layui-icon layui-icon-add-1"></i> 添加条件</a><a class="layui-btn layui-btn-sm" style="background-color: #2D93CA" data-type="addGroup"><i class="layui-icon layui-icon-add-circle" ></i> 添加分组</a><a class="layui-btn layui-btn-sm" data-type="search" style="float: right; background-color: #2D93CA"><i class="layui-icon layui-icon-search"></i> 查询</a><span style="float: right"><input type="checkbox" lay-filter="out_auto" class="out_auto" title="实时更新" style="background-color: #2D93CA"></span></div>')
      filterBoard.push('<hr>')
      filterBoard.push('<ul>')
      for (i = 0; i < filterSos.length; i++) {
      ...
      function groupHtml(filterSo, filterBoard, fieldMap, isFirst, isLast) {
        ...
        switch (mode) {
          ...
          case 'condition':
            ...
            break;
          case 'group':
            filterBoard.push('<div class="layui-firebrick" style="background-color: #2D93CA">分组</div>')
            filterBoard.push('<div><a class="layui-btn layui-btn-xs" data-type="addOne" style="background-color: #2D93CA"><i class="layui-icon layui-icon-add-1"></i> 添加条件</a><a class="layui-btn layui-btn-xs" data-type="addGroup" style="background-color: #2D93CA"><i class="layui-icon layui-icon-add-circle"></i> 添加分组</a></div>')
            filterBoard.push('<div class="layui-red delete-item"><i class="layui-icon layui-icon-close-fill"></i></div>');
